...
def results():
    ...
    context = {
        ...
        'sunrise': datetime.fromtimestamp(timestamp=result_json['sys']['sunrise']),
        'sunset': datetime.fromtimestamp(timestamp=result_json['sys']['sunset']),
...
def comparison_results():
    ...
    city1_result = requests.get(API_URL, params=params).json()
    city2_result = requests.get(API_URL, params=params).json()
    ...
    context = {
        ...
        'city1_info': {
            ...
            'sunset': datetime.fromtimestamp(timestamp=city1_result['sys']['sunset']),
            'description': city1_result['weather'][0]['description'],
            'min_temp': city1_result['main']['temp_min'],
            'max_temp': city1_result['main']['temp_max']
        },
        'city2_info': {
            'temp': city2_result['main']['temp'],
            'humidity': city2_result['main']['humidity'],
            'wind_speed': city2_result['wind']['speed'],
            'sunset': datetime.fromtimestamp(timestamp=city2_result['sys']['sunset']),
            'description': city2_result['weather'][0]['description'],
            'min_temp': city2_result['main']['temp_min'],
            'max_temp': city2_result['main']['temp_max']
        },
        'units_letter': get_letter_for_units(units)
    }
